...
if (args[0] === "init") {
  ...
  server.registerTool(
    ...
    async ({ url, fileKey, nodeId, depth }) => {
      try {
        ...
        const output = texts.map((t) => `[${t.path}] ${t.text}${t.style ? ` (${t.style})` : ""}`).join("\n");
  ...
  server.registerTool(
    ...
    async ({ fileKey }) => {
      try {
        ...
        return {
          content: [
            { type: "text" as const, text: `# 版本历史 (共 ${data.versions.length} 个)\n\n${lines.join("\n")}` },
          ],
        };
      } catch (error) {
        return formatError(error);
  ...
  server.registerTool(
    ...
    {
      description:
        "对比两个节点的差异,或对比同一节点的前后变化。支持 snapshot 模式（通过 Figma 版本历史获取之前的版本进行对比）和 nodes 模式（两个不同节点直接对比）",
      inputSchema: {
        fileKey: z.string().describe("Figma 文件 Key"),
        ...
        mode: z
          ...
          .optional()
          .default("nodes")
          .describe("对比模式:snapshot（与历史版本对比）或 nodes（两节点对比,默认）"),
        ...
        versionId: z
          .string()
          .optional()
          .describe("mode=snapshot 时可选,指定要对比的历史版本 ID。不传则自动使用上一个版本"),
      ...
      try {
        ...
        if (mode === "snapshot") {
          ...
          if (!previousData?.nodes?.[normalizedId]) {
            return {
              content: [{ type: "text" as const, text: `历史版本中节点 ${normalizedId} 不存在（可能是新增节点）` }],
            };
          }
          ...
          return {
            content: [
              {
                type: "text" as const,
                text: `# 节点变化: ${nodeB.name} (${normalizedId})\n\n对比版本: ${versionLabel} → 当前\n\n${output}`,
              },
            ],
          };
        }
      ...
      if (index.icons.length === 0) {
        return {
          content: [
            {
              type: "text" as const,
              text: "当前会话尚未导出任何图标。使用 get_node 或 export_svg 工具导出图标后，索引会自动更新。",
            },
          ],
        };
      }

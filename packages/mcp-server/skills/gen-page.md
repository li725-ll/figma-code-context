...
在拆分或生成任何新组件之前，先盘点项目中已有的组件：

1. 扫描项目组件目录（src/components/、components/、app/components/ 等），列出已有组件：
   - 组件名称
   - Props 接口（关键 props）
   - 文件路径
2. 将 Figma 设计中可识别的 UI 单元与已有组件做匹配：
   - 名称匹配（Figma Component 名 vs 项目组件名）
   - 结构/功能匹配（相似的 props、变体、视觉结构）
3. 匹配结果分三类处理：
   - **直接复用** — 已有组件完全满足设计需求 → 直接 import 使用，不重新生成
   - **扩展复用** — 已有组件基本满足但缺少某个 variant/prop → 修改已有组件，新增所需变体
   - **新建** — 项目中确实不存在对应组件 → 才新建
4. 如果不确定是否匹配，询问用户："项目中已有 `<ComponentName>` 组件，是否复用？"

### 原则

- **复用优先**：能用已有组件的绝不新建
- **扩展优于新建**：已有组件缺少变体时，优先扩展而非另起炉灶
- **保持一致性**：复用已有组件能确保项目 UI 风格统一
